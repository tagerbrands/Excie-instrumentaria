            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {interviews.sort((a, b) => new Date(b.lastUpdated).valueOf() - new Date(a.lastUpdated).valueOf()).map((interview) => {
                  const isAn = interview.isAnalysis;
                  return (
                  <tr key={interview.id} className={`transition-colors ${isAn ? 'bg-purple-50 hover:bg-purple-100 dark:bg-purple-900/20 dark:hover:bg-purple-900/40 border-l-4 border-l-purple-500' : 'hover:bg-gray-50/50 dark:hover:bg-gray-700/50'}`}>
                    ...
                    <td className="px-6 py-4" onClick={() => toggleSelection(interview.id)}>
                      <span className="font-medium text-gray-900 dark:text-gray-100 cursor-pointer">
                        {isAn ? (interview.analysisTitle || 'Analyse') : (interview.excie || 'Onbekend')}
                        {isAn && <span className="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-800 dark:bg-purple-800 dark:text-purple-100">Analyse</span>}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-gray-600 dark:text-gray-400">{isAn ? '-' : (interview.datum || '-')}</td>
                    <td className="px-6 py-4 text-gray-600 dark:text-gray-400">{isAn ? '-' : (interview.cveLid || '-')}</td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex justify-end gap-2">
                        <button 
                          ...
                          className={`p-2 rounded-md transition-colors ${isAn ? 'text-purple-600 hover:bg-purple-200 dark:text-purple-400 dark:hover:bg-purple-800' : 'text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 bg-gray-50 dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-blue-900/30'}`}
                          title="Bewerken"
                        >
                          <Edit2 size={16} />
                        </button>
                        <button 
                          ...
                          className={`p-2 rounded-md transition-colors ${isAn ? 'text-purple-600 hover:bg-red-200 dark:text-purple-400 dark:hover:bg-red-900/30' : 'text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 bg-gray-50 dark:bg-gray-800 hover:bg-red-50 dark:hover:bg-red-900/30'}`}
                          title="Verwijderen"
                        >
                          <Trash2 size={16} />
                        </button>
